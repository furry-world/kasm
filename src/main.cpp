...
int main(int argc, char **argv) {
    ...
    args::Positional<std::string> input_filename(
        argument_parser,
        "input",
        "Set input file name"
    );
    ...
    if (input_filename) {
        input_file_path =
            std::filesystem::path(args::get(input_filename).c_str());
    } else {
        std::cerr << STRINGS_ERROR_PREFIX "You need to specify an input file."
                  << std::endl;
        std::cout << argument_parser;
        return 1;
    }
    ...
    if (output_filename) {
        output_file_path =
            std::filesystem::path(args::get(output_filename).c_str());
    } else {
        output_file_path =
            std::filesystem::path(args::get(input_filename).c_str());
